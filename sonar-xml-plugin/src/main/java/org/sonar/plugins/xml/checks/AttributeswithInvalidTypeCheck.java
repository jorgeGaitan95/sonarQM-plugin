...
@Rule(key="AttributesWithInvalidTypeCheck",
name="The Attribute must have a valid type: (VARCHAR, NUMBER, DATE, AUTO, OLE, BOOELAN)",
priority= Priority.BLOCKER)
@BelongsToProfile(title=CheckRepository.SONAR_WAY_PROFILE_NAME,priority=Priority.BLOCKER)
@SqaleSubCharacteristic(RulesDefinition.SubCharacteristics.ERRORS)
@SqaleConstantRemediation("30min")
public class AttributeswithInvalidTypeCheck extends AbstractXmlCheck {
		...
		for (int i = 0; i < length; i++) {
			...
			if(nodoAux.getNodeType()==Node.ELEMENT_NODE&&nodoAux.getNodeName().equals(getVariables().NODE_ATTRIBUTE_ER))
			{
				...
				if(attribute.getNamedItem(getVariables().ATTRIBUTE_TYPE_ATTRIBUTENODE)!=null){
					...
					if(tipo.equals("VARCHAR")||tipo.equals("NUMBER")||tipo.equals("DATE")||tipo.equals("AUTO")||tipo.equals("OLE")||tipo.equals("BOOLEAN")){
